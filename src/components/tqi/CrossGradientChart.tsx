import { ComposedChart, Bar, ResponsiveContainer, ReferenceLine, ReferenceArea, XAxis, YAxis, Tooltip } from "recharts";
...
export function CrossGradientChart({ series, nominal, tolerance }: Props) {
  ...
  const domain: [number, number] = [0, Math.max(nominal + tolerance * 2 + 1, 8)];
    ...
    <div className="panel p-4 h-full flex flex-col">
      <div className="mb-2">
        ...
        <p className="text-[12px] text-foreground-muted mt-0.5">
          |Front − Rear| · Nominal {nominal}°C · Tolerance ±{tolerance}°C · Now {last?.delta.toFixed(1)}°C
        </p>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={series} margin={{ top: 32, right: 90, bottom: 28, left: 4 }}>
            ...
            <YAxis
              domain={domain}
              tick={{ fill: "hsl(var(--foreground-dim))", fontSize: 10 }}
              label={{ value: "Delta Temp (°C)", angle: -90, position: "insideLeft", fill: "hsl(var(--foreground-dim))", fontSize: 11 }}
              stroke="hsl(var(--border))"
            />
            <Tooltip
            ...
            {/* Tolerance band */}
            <ReferenceArea
              y1={Math.max(0, nominal - tolerance)}
              y2={nominal + tolerance}
            ...
            <ReferenceLine
              ...
              stroke="hsl(var(--status-good))"
              strokeDasharray="6 4"
              label={{ value: `Nominal ${nominal}°C`, fill: "hsl(var(--status-good))", fontSize: 10, position: "insideTopRight" }}
            />
            <ReferenceLine
              y={nominal + tolerance}
              ...
              strokeDasharray="3 3"
              strokeOpacity={0.7}
              label={{ value: `+${tolerance}°C limit`, fill: "hsl(var(--status-warn))", fontSize: 9, position: "insideBottomRight" }}
            />
            <Bar
              dataKey="delta"
              name="|Front − Rear| (°C)"
