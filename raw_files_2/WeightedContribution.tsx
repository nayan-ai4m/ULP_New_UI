  ...
  return (
    <div className="panel p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[10px] font-mono text-foreground-dim">Σ = {pqi.toFixed(2)}</span>
      </div>

      <div className="grid grid-cols-3 gap-2">
        {items.map((it) => {
          const pctOfMax = (it.weighted / it.weight) * 100; // fill of own cap
          return (
            <div
              key={it.key}
              className={cn(
            ...
            >
              <div className="text-[9px] uppercase tracking-wider opacity-80">{it.label}</div>
              <div className="font-mono text-xl font-semibold leading-none mt-1 text-foreground">
                {it.weighted.toFixed(2)}
              </div>
              <div className="text-[10px] text-foreground-muted mt-1 leading-tight">
                of {it.weight.toFixed(2)} cap
              </div>
              <div className="text-[10px] font-mono text-foreground-dim mt-1.5">
                {it.rawLabel}: {it.raw}{it.rawUnit}
              </div>
      ...
      <div className="mt-4">
        <div className="text-[10px] uppercase tracking-wider text-foreground-dim mb-1.5">
          PQI composition
        </div>
